...
ALLOCATEMBUF:
	...
	push	[BP+bsize2]
	call	HMEM_ALLOCBYTE
	mov	word ptr [DI].SPART.mbuf+2,AX
	mov	word ptr [DI].SPART.mbuf,0
...
ALLOCATESBUF:
	...
	push	(SBUFMAX * 2)+1
	call	HMEM_ALLOCBYTE
	mov	word ptr [DI].SESOUND.sbuf+2,AX
	mov	word ptr [DI].SESOUND.sbuf,0
	jnc	short SBUFALLOCATED
	push	word ptr [DI].SPART.mbuf+2
	mov	word ptr [DI].SPART.mbuf+2,0
	call    HMEM_FREE
	mov	AX,BGM_OVERFLOW
	MRETURN
SBUFALLOCATED:
	...
	;BGMOFF
	mov	glb.SGLB.rflg,OFF
	;登録曲数
	mov	glb.SGLB.mnum,0
	;セレクト中曲番号
	mov	glb.SGLB.mcnt,0
	;パート数
	mov	glb.SGLB.pnum,PMAX
	;パートカウンタ
	mov	glb.SGLB.pcnt,0
	;終了パート数
	mov	glb.SGLB.fin,0
	;リピートON
	mov	glb.SGLB.repsw,ON
	;処理カウンタ
	mov	glb.SGLB.tcnt,0
	;曲バッファ末尾 
	mov	glb.SGLB.buflast,0
	;曲ごとのテンポ情報 

	CLD

	mov	CX,MMAX
	mov	DI,offset glb.SGLB.mtp
	push	DS
	pop	ES
	mov	AX,DEFTEMPO
	rep 	stosw
	;効果音OFF
	mov	glb.SGLB.effect,OFF
	;登録効果音数
	mov	glb.SGLB.snum,0
	;セレクト中効果音番号
	mov	glb.SGLB.scnt,0
	;演奏処理ON
	mov	glb.SGLB.music,ON
	;効果音処理ON
	mov	glb.SGLB.sound,ON
...
SET_CLOCKBASE:
	...
	mov	DX,DEFTEMPO
	mul	DX
	mov	word ptr glb.SGLB.clockbase+2,DX
	mov	word ptr glb.SGLB.clockbase,AX

	push	DEFTEMPO
	call	BGM_SET_TEMPO

	;バッファクリア
	mov	BX,offset part.SPART.mbuf
	xor	DX,DX
	xor	AX,AX
BUFFERCLEARLOOP:
	les	DI,[BX]
	mov	CX,glb.SGLB.bufsiz
	shr	CX,1
	rep	stosw
	adc	CX,CX
	rep	stosb
...
NOTINITIALIZED2:
	...
	mov	SI,offset part
FREEMBUF:
	mov	AX,word ptr [SI].SPART.mbuf+2
	cmp	AX,0
	je	short NOMBUF
	push	AX
	call	HMEM_FREE
	mov	word ptr [SI].SPART.mbuf+2,0
NOMBUF:
	add	SI,type SPART
...
FREESBUF:
	...
	mov	AX,word ptr [SI].SESOUND.sbuf+2
	cmp	AX,0
	je	short NOSBUF
	push	AX
	call	HMEM_FREE
	mov	word ptr [SI].SESOUND.sbuf+2,0
NOSBUF:
	add	SI,type SESOUND
