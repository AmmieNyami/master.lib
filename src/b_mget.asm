...
even
KYUUFU:
	mov	byte ptr [SI].SPART.note,REST
	jmp	short NOTELENGTH

	;音符
even
ONPU:
	mov	AL,byte ptr [SI].SPART.note
	sub	AL,'A'
	cmp	AL,'G' - 'A'
	ja	short WHILEEND
even
...
SHARP:
	;シャープだった
	add	byte ptr [SI].SPART.note,DIVNUM
	inc	word ptr [SI].SPART.pptr
NOTSHARP:
	les	BX,[SI].SPART.pptr
	cmp	byte ptr ES:[BX],'-'
	...
	;フラットだった
	add	byte ptr [SI].SPART.note,DIVNUM*2
	inc	word ptr [SI].SPART.pptr
