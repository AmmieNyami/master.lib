...
;	register SI = mute
	...
	mov	AX,glb.SGLB.pcnt
	imul	BX,AX,type SPART
	cmp	part[BX].SPART.lcnt,MINLCNT
	jne	short MUTEISFALSE
	cmp	part[BX].SPART.tnt,ON
	je	short MUTEISFALSE
	cmp	part[BX].SPART.len,MINNOTE
	je	short MUTEISFALSE
	mov	SI,TRUE
...
PART0:
	...
	cmp	byte ptr part[BX].SPART.note,REST
	je	short PART0_NOT_BELL_PART0
	or	SI,SI
	je	short BELL_PART0
PART0_NOT_BELL_PART0:
	cmp	byte ptr part[BX+type SPART].SPART.note,REST
	je	short PART0_NOT_BELL_PART1
	cmp	part[BX+type SPART].SPART.lcnt,MINLCNT
	jg	short BELL_PART1
PART0_NOT_BELL_PART1:
	cmp	byte ptr part[BX+type SPART*2].SPART.note,REST
	je	short BELL_PART0
	cmp	part[BX+type SPART*2].SPART.lcnt,MINLCNT
	jg	short BELL_PART2
...
NOBELL:
	...
	;音長カウンタデクリメント
	imul	BX,glb.SGLB.pcnt,type SPART
	dec	part[BX].SPART.lcnt
...
NOTPARTEND:
	...
	inc	glb.SGLB.pcnt
	cmp	glb.SGLB.pcnt,AX
	jne	short NOTALLPARTEND
	mov	AX,word ptr [BP+_rp]
	mov	glb.SGLB.pcnt,AX

	;全てのパートが終了したらパート初期化
	cmp	glb.SGLB.fin,ALL_PART
	jne	short NOTALLPARTEND
