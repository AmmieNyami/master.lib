...
OPENED:
	mov	SI,AX
	mov	DI,glb.SGLB.bufsiz
	shl	DI,1
	add	DI,glb.SGLB.bufsiz			;glb.bufsiz*3
	push	DI
	call	SMEM_WGET
	jc	short NOMEM
	mov	[BP+bufp+2],AX
...
SKIP_SPACE:
	...
	mov	SI,BX
	mov	AX,SS:[BX]
	add	AX,glb.SGLB.buflast
	cmp	AX,glb.SGLB.bufsiz
	jl	short PUTBUFFER
	jmp	short BREAK
PUTBUFFER:
	...
	les	BX,part[BX].SPART.mbuf
	add	BX,SS:[SI]
	mov	DI,glb.SGLB.buflast
	mov	ES:[BX][DI],AL
	inc	word ptr SS:[SI]
...
END_ANALISYS:
	...
	mov	word ptr [BP+tmp],offset part.SPART.mbuf
	lea	SI,[BP+cnt]
...
NO_OVERFLOW:
	...
	mov	AX,glb.SGLB.buflast
	mov	SI,glb.SGLB.mnum
	dec	SI
	shl	SI,1
	mov	glb.SGLB.track[SI],AX
	;glb.buflast += (maxpart + 1);
	lea	AX,[BX+1]
	add	glb.SGLB.buflast,AX
...
INITPART:
	...
	mov	glb.SGLB.tp,DI
	mov	BX,glb.SGLB.mnum
	dec	BX
	shl	BX,1
	mov	glb.SGLB.mtp[BX],DI
	;glb.tval = (uint)(glb.clockbase / (ulong)glb.tp);
	mov	DX,word ptr glb.SGLB.clockbase+2
	mov	AX,word ptr glb.SGLB.clockbase
	div	DI
	mov	glb.SGLB.tval,AX
INVALID_TEMPO:
