...
;	mov	word ptr esound[BX].sptr,AX
	mov	word ptr esound[BX].SESOUND.sptr,0	; offsetは0のはず
	mov	AX,word ptr esound[BX].SESOUND.sbuf+2
	mov	word ptr esound[BX].SESOUND.sptr+2,AX
...
NUMBER_END:
	...
	mov	BX,glb.SGLB.snum
	shl	BX,3
	mov	DX,BX
	les	BX,esound[BX].SESOUND.sptr
	mov	ES:[BX],CX			; *esound[glb.senum].sptr = data
	mov	BX,DX
	add	word ptr esound[BX].SESOUND.sptr,2	; esound[glb.senum].sptr++
...
;	mov	word ptr esound[BX].sptr,CX
	mov	word ptr esound[BX].SESOUND.sptr,0	; offsetは0のはず
	mov	CX,word ptr esound[BX].SESOUND.sbuf+2
	mov	word ptr esound[BX].SESOUND.sptr+2,CX
NOT_SBUFMAX:
	cmp	AL,0ffh
	je	short LOOP_BREAK
	jmp	LOOP_TOP
