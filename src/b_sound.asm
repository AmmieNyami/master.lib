...
ILLEGAL:
	...
	ret	2
OK:
	cmp	glb.SGLB.sound,ON
	jne	short NOSOUND
	call	_BGM_BELL_ORG
	mov	glb.SGLB.scnt,SI
	;esound[glb.scnt - 1].sptr = esound[glb.scnt - 1].sbuf;
	mov	BX,SI
	shl	BX,3
	mov	AX,offset esound-8
	add	BX,AX
	mov	AX,word ptr [BX].SESOUND.sbuf+2
	mov	word ptr [BX].SESOUND.sptr+2,AX
;	mov	AX,word ptr [BX].sbuf
;	mov	word ptr [BX].sptr,AX
	mov	word ptr [BX].SESOUND.sptr,0	;sbufのoffsetは0のはず
	mov	glb.SGLB.effect,ON
NOSOUND:
