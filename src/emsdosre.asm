...
YESMEM:
	...
	mov	[bp + read_counter].FP.FP_SEG, ax
	mov	[bp + read_counter].FP.FP_OFF, ax
LOOP_TOP:
	mov	ax, [bp + read_bytes].FP.FP_OFF
	mov	dx, [bp + read_bytes].FP.FP_SEG
	sub	ax, [bp + read_counter].FP.FP_OFF
	sbb	dx, [bp + read_counter].FP.FP_SEG
	or	dx, dx
...
READ_FROM_FILE:
	...
	add	[bp + read_counter].FP.FP_OFF, ax
	adc	[bp + read_counter].FP.FP_SEG, dx
	mov	ax, [bp + read_bytes].FP.FP_OFF
	mov	dx, [bp + read_bytes].FP.FP_SEG
	cmp	[bp + read_counter].FP.FP_SEG, dx
	jl	short LOOP_TOP
	jg	short LOOP_END
	cmp	[bp + read_counter].FP.FP_OFF, ax
	jb	short LOOP_TOP
LOOP_END:
	push	[bp + local_buf].FP.FP_SEG
	_call	SMEM_RELEASE
	mov	ax, [bp + read_counter].FP.FP_OFF
	mov	dx, [bp + read_counter].FP.FP_SEG
	mov	sp, bp
	pop	bp
	ret	12
endfunc			; }
