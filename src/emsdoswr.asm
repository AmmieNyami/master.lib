...
NOMEM:
	...
	ret	12
YESMEM:
	mov	[bp + local_buf].FP.FP_SEG, ax
	sub	ax, ax
	mov	[bp + local_buf].FP.FP_OFF, ax
	mov	[bp + write_counter].FP.FP_SEG, ax
	mov	[bp + write_counter].FP.FP_OFF, ax
LOOP_TOP:
	mov	ax, [bp + write_bytes].FP.FP_OFF
	mov	dx, [bp + write_bytes].FP.FP_SEG
	sub	ax, [bp + write_counter].FP.FP_OFF
	sbb	dx, [bp + write_counter].FP.FP_SEG
	or	dx, dx
...
READ_FROM_FILE:
	push	[bp + ems_handle].FP.FP_OFF
	push	[bp + ems_offs].FP.FP_SEG
	push	[bp + ems_offs].FP.FP_OFF
	push	[bp + local_buf].FP.FP_SEG
	push	[bp + local_buf].FP.FP_OFF
	...
	sub	dx, dx
	add	[bp + ems_offs].FP.FP_OFF, ax
	adc	[bp + ems_offs].FP.FP_SEG, dx
	add	[bp + write_counter].FP.FP_OFF, ax
	adc	[bp + write_counter].FP.FP_SEG, dx
	mov	ax, [bp + write_bytes].FP.FP_OFF
	mov	dx, [bp + write_bytes].FP.FP_SEG
	cmp	[bp + write_counter].FP.FP_SEG, dx
	jl	short LOOP_TOP
	jg	short LOOP_END
	cmp	[bp + write_counter].FP.FP_OFF, ax
	jb	short LOOP_TOP
LOOP_END:
	push	[bp + local_buf].FP.FP_SEG
	_call	SMEM_RELEASE
	mov	ax, [bp + write_counter].FP.FP_OFF
	mov	dx, [bp + write_counter].FP.FP_SEG
	mov	sp, bp
	pop	bp
	ret	12
endfunc			; }
