...
func FONT_ENTRY_BFNT	; font_entry_bfnt() {
	...
	mov	AL,[BP+header].bfnt_header.col
	cmp	AL,0			; 2色パレットなしでなければならない
	jne	short READ_ERROR

	mov	AH,byte ptr [BP+header].bfnt_header.Xdots
	add	AH,7
	mov	CL,3
	shr	AH,CL			; Xdotsは 8で割る
	mov	AL,byte ptr [BP+header].bfnt_header.Ydots
	test	AX,AX
	mov	DX,AX
	jz	short READ_ERROR
	cmp	font_AnkSeg,0
...
SKIP_ALLOCATE:
	...
	mov	BX,AX
	mov	AX,[BP+header].bfnt_header.START
