...
BAK_1PAGE:
	...
	mov	word ptr [BP+trans].EMSSTRUCT.region_length,8000h
	mov	word ptr [BP+trans].EMSSTRUCT.region_length+2,0

	mov	[BP+trans].EMSSTRUCT.source_memory_type,0	; from Main Memory

	mov	[BP+trans].EMSSTRUCT.source_initial_seg_page,0a800H

	mov	[BP+trans].EMSSTRUCT.dest_memory_type,1	; to EMS

	mov	AX,vram_handle
	mov	[BP+trans].EMSSTRUCT.dest_handle,AX

	xor	AX,AX
	mov	[BP+trans].EMSSTRUCT.source_handle,AX
	mov	[BP+trans].EMSSTRUCT.source_initial_offset,AX
	mov	[BP+trans].EMSSTRUCT.dest_initial_offset,AX
	mov	[BP+trans].EMSSTRUCT.dest_initial_seg_page,AX
...
BAK_LOOP:
	...
	add	[BP+trans].EMSSTRUCT.dest_initial_seg_page,2
...
RES_START:
	...
	mov	[BP+trans].EMSSTRUCT.source_memory_type,1

	mov	AX,vram_handle
	mov	[BP+trans].EMSSTRUCT.source_handle,AX

	xor	AX,AX
	mov	[BP+trans].EMSSTRUCT.dest_memory_type,AL

	mov	[BP+trans].EMSSTRUCT.source_initial_offset,AX
	mov	[BP+trans].EMSSTRUCT.source_initial_seg_page,AX
	mov	[BP+trans].EMSSTRUCT.dest_handle,AX
	mov	[BP+trans].EMSSTRUCT.dest_initial_offset,AX

	mov	[BP+trans].EMSSTRUCT.dest_initial_seg_page,0a800H
...
RES_LOOP:
	...
	add	[BP+trans].EMSSTRUCT.source_initial_seg_page,2
