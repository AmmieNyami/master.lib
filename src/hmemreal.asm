...
hmem_reallocb:
	...
	jae	short NO_GOOD
	mov	ES,CX
	cmp	ES:[SI].MEMHEAD.using,1	; usingフラグが「使用中(1)」でなければ zero ret
	jne	short NO_GOOD

	; さて本番
	mov	AX,ES:[SI].MEMHEAD.mem_id	; IDを転写しておく
	mov	mem_AllocID,AX

	mov	AX,ES:[SI].MEMHEAD.nextseg ; AX=nextseg
	...
	mov	ES,AX
	cmp	ES:[SI].MEMHEAD.using,SI
	jne	short OTHER_PLACE	; 次のブロックが使用中ならやはり移動

	; 後ろに空間がある
	cmp	ES:[SI].MEMHEAD.nextseg,BX
	jb	short OTHER_PLACE	; 後ろの空間を足しても足りない?
	; 足りる場合
	cmp	AX,mem_FirstHole
	jne	short CONNECT
	; FirstHoleをつぶすために、確保するのだ
	sub	AX,ES:[SI].MEMHEAD.nextseg
	not	AX			; (nextseg - freeseg) - 1
	push	AX
	call	HMEM_ALLOC
CONNECT:
	mov	AX,ES:[SI].MEMHEAD.nextseg
	mov	ES,CX
	mov	ES:[SI].MEMHEAD.nextseg,AX	; 結合する
	cmp	AX,BX
SHRINK:
...
ENDIF
	...
	; 縮める
	mov	ES:[SI].MEMHEAD.nextseg,BX	; BX(理想の次位置)を実現する
	mov	ES,BX
	mov	ES:[SI].MEMHEAD.nextseg,AX	; その地点の次位置
	mov	ES:[SI].MEMHEAD.using,1		; 使用中のブロック二つに分断
...
OTHER_PLACE:
	...
	mov	ES,CX
	mov	ES:[SI].MEMHEAD.nextseg,AX
	mov	ES:[SI].MEMHEAD.using,1
	push	mem_AllocID
	pop	ES:[SI].MEMHEAD.mem_id
	mov	mem_TopHeap,CX
	add	BX,CX
	sub	AX,BX
	; CX=新しい位置(管理領域の位置)
