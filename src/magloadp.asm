...
READPALETTE:
	...
	mov	AX,ES:[DI].MagHeader._x2
	or	AX,7
	mov	SI,ES:[DI].MagHeader._x1
	and	SI,not 7
	sub	AX,SI
	jc	short INVALID_RETURN	; 横の長さがマイナスじゃあねえ
	inc	AX
	mov	ES:[DI].MagHeader._xsize,AX	; 横の長さ格納

IF 0	; 256色はまだねえ…
	mov	CL,2
	test	ES:[DI].MagHeader._scrnmode,80h
	jnz	short COLOR256
	inc	CX			; でもパレットが…
COLOR256:
	...
...
ENDIF
	...
	mov	CX,ES:[DI].MagHeader._y2
	sub	CX,ES:[DI].MagHeader._y1
	jc	short INVALID_RETURN	; 縦の長さがマイナスじゃあねえ
	inc	CX
	mov	ES:[DI].MagHeader._ysize,CX	; 縦の長さ格納
	mov	[BP+lines],CX
...
DATA_OK:
	...
	mov	AX,word ptr ES:[DI].MagHeader._flagBofs
	mov	DX,word ptr ES:[DI+2].MagHeader._flagBofs
	sub	AX,word ptr ES:[DI].MagHeader._flagAofs
	sbb	DX,word ptr ES:[DI+2].MagHeader._flagAofs
	...
	mov	AX,WORD PTR ES:[DI].MagHeader._pixelofs
	mov	DX,WORD PTR ES:[DI+2].MagHeader._pixelofs
	sub	AX,WORD PTR ES:[DI].MagHeader._flagBofs
	sbb	DX,WORD PTR ES:[DI+2].MagHeader._flagBofs
	mov	[BP+flagBsize],AX
