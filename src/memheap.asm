...
SEARCH_HOLE:
	mov	ES,AX
	mov	AX,(MEMHEAD PTR ES:[0]).nextseg
	cmp	(MEMHEAD PTR ES:[0]).using,0
...
ALLOC_CENTER:
	...
	mov	mem_TopHeap,AX
	mov	ES,AX
	mov	(MEMHEAD PTR ES:[0]).nextseg,CX
	mov	(MEMHEAD PTR ES:[0]).using,1
	mov	BX,AX
	jmp	short RETURN
...
FOUND_HOLE:
	...
	mov	BX,ES
	mov	ES,CX
	mov	(MEMHEAD PTR ES:[0]).nextseg,AX
	mov	(MEMHEAD PTR ES:[0]).using,0
...
RETURN:	mov	ES,BX
	mov	AX,0
	xchg	AX,mem_AllocID
	mov	(MEMHEAD PTR ES:[0]).mem_id,AX
	lea	AX,[BX+1]
	clc
