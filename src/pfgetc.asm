...
func PFGETC	; pfgetc() {
	...
	call	word ptr ES:[PFILE.pf_getc]
...
PFGETC1 proc near	; pfgetc1() {
	cmp	ES:[PFILE.pf_cnt],0
	jz	short PFGETC1_getx
	dec	ES:[PFILE.pf_cnt]
	add	word ptr ES:[PFILE.pf_loc],1
	adc	word ptr ES:[PFILE.pf_loc + 2],0
	mov	AX,ES:[PFILE.pf_ch]
	ret

PFGETC1_getx:
	call	word ptr ES:[PFILE.pf_getx]		; 新しいバイトが
	test	AH,AH
	jnz	short PFGETC1_return

	cmp	AX,ES:[PFILE.pf_ch]		; 直前のバイトと同じ値なら
	mov	ES:[PFILE.pf_ch],AX
	jne	short PFGETC1_return

	push	AX
	call	word ptr ES:[PFILE.pf_getx]		; くり返しだ。続くバイトが
	test	AH,AH
	jnz	short PFGETC1_return1
	mov	ES:[PFILE.pf_cnt],AX		; 長さね。
	sub	word ptr ES:[PFILE.pf_loc],1
	sbb	word ptr ES:[PFILE.pf_loc + 2],0
PFGETC1_return1:
	pop	AX
...
PFGETX0 proc near	; pfgetx0() {
	mov	AX,word ptr ES:[PFILE.pf_read]
	mov	DX,word ptr ES:[PFILE.pf_read + 2]
	cmp	DX,word ptr ES:[PFILE.pf_size + 2]
	jb	short PFGETX0_getc
	ja	short PFGETX0_eof
	cmp	AX,word ptr ES:[PFILE.pf_size]
	jb	short PFGETX0_getc
PFGETX0_eof:
	mov	AX,EOF
	ret
...
PFGETX0_getc:
	...
	mov	word ptr ES:[PFILE.pf_read + 2],DX
