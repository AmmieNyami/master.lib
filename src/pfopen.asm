...
func PFOPEN	; pfopen() {
	...
	test	SS:[DI.pfHeader.hd_aux],PF_BODY_ENCRYPTED
	jz	short PFOPEN_1
	mov	AX,offset PFGETX1
PFOPEN_1:
	mov	word ptr ES:[PFILE.pf_getx],AX
	;	pf系関数はすべて同じセグメントである

	; 復号化用キー
	mov	AL,pfkey
	mov	ES:[PFILE.pf_key],AL

	mov	AL,SS:[DI.pfHeader.hd_type]
	cmp	AL,PF_NONE
...
PFOPEN_pf_none:
	mov	AX,word ptr ES:[PFILE.pf_getx]
	mov	word ptr ES:[PFILE.pf_getc],AX
	jmp	short PFOPEN_return

PFOPEN_pf_len:
	mov	word ptr ES:[PFILE.pf_getc],offset PFGETC1
	mov	ES:[PFILE.pf_cnt],0
	mov	ES:[PFILE.pf_ch],EOF

PFOPEN_return:
	mov	AX,word ptr SS:[DI.pfHeader.hd_psz]
	mov	DX,word ptr SS:[DI.pfHeader.hd_psz + 2]
	mov	word ptr ES:[PFILE.pf_size],AX
	mov	word ptr ES:[PFILE.pf_size + 2],DX

	mov	AX,word ptr SS:[DI.pfHeader.hd_osz]
	mov	DX,word ptr SS:[DI.pfHeader.hd_osz + 2]
	mov	word ptr ES:[PFILE.pf_osize],AX
	mov	word ptr ES:[PFILE.pf_osize + 2],DX

	xor	AX,AX
	mov	word ptr ES:[PFILE.pf_read],AX
	mov	word ptr ES:[PFILE.pf_read + 2],AX
	mov	word ptr ES:[PFILE.pf_loc],AX
	mov	word ptr ES:[PFILE.pf_loc + 2],AX
...
SEEK	proc near	; {
	...
	mov	ES,SI
	xor	AX,AX
	mov	word ptr ES:[PFILE.pf_home],AX
	mov	word ptr ES:[PFILE.pf_home + 2],AX
...
STRRCHR_DONE:
	...
	jne	SEEK_error
	mov	BX,10
	cmp	[DI.exeHeader.eh_M],'M'
	jne	SEEK_error
	mov	BX,11
	cmp	[DI.exeHeader.eh_Z],'Z'
	jne	SEEK_error

	mov	AX,[DI.exeHeader.eh_page]
	dec	AX
	mov	DX,512
	mul	DX
	add	AX,[DI.exeHeader.eh_mod]
	;adc	DX,0	; いらない

	mov	ES,SI
	mov	word ptr ES:[PFILE.pf_home],AX
	mov	word ptr ES:[PFILE.pf_home + 2],DX
...
SEEK_loop:
	...
	mov	ES,SI
	add	word ptr ES:[PFILE.pf_home],AX
	adc	word ptr ES:[PFILE.pf_home + 2],0

	; ヘッダの正当性をチェック
	mov	AL,SS:[DI.pfHeader.hd_type]
	xor	AL,SS:[DI.pfHeader.hd_chk]
	inc	AL
	mov	BX,PFEILPFILE
	jnz	short SEEK_error

	; ファイル名が不可視の場合、比較のためもとに戻す
	test	SS:[DI.pfHeader.hd_aux],PF_FNAME_INVISIBLE
	jz	short SEEK_cmp
...
SEEK_cmp:
	...
	mov	DX,word ptr SS:[DI.pfHeader.hd_psz + 2]
	add	word ptr ES:[PFILE.pf_home],AX
	adc	word ptr ES:[PFILE.pf_home + 2],DX
