...
func SUPER_RESTORE_EMS	; super_restore_ems() {
	...
	cmp	SI,-1
	jne	short RESTORE_1
	mov	SI,(BACKUP_RECORD PTR ES:[0]).top_number
RESTORE_1:
	mov	DI,(BACKUP_RECORD PTR ES:[0]).num_data
	...
	mov	AX,word ptr (BACKUP_RECORD PTR ES:[0]).top_offset
	mov	[BP+read_off],AX
	mov	AX,word ptr (BACKUP_RECORD PTR ES:[0]).top_offset+2
	mov	[BP+read_off+2],AX

	mov	word ptr [BP+read_adr],BACKUP_RECORD.patsize
	mov	AX,[BP+handle]
