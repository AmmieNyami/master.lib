...
func WFONT_ENTRY_BFNT	; wfont_entry_bfnt() {
	...
	cmp	[BP+header].bfnt_header.Xdots,8	; 8x8dotモノクロ以外ではInvalidData
	jne	short DATA_ERROR
	cmp	[BP+header].bfnt_header.Ydots,8
	jne	short DATA_ERROR
	cmp	[BP+header].bfnt_header.col,0
	jne	short DATA_ERROR
...
SKIP_ALLOCATE:
	...
	mov	BX,[BP+header].bfnt_header.START
	mov	CX,[BP+header].bfnt_header.END_
	sub	CX,BX
	inc	CX
	mov	BH,0
	add	AX,BX
